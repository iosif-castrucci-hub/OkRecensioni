...
// ===================== CATEGORY DETECTION =====================
const CATEGORY_MAP = [
  { test: /ristorant|trattor|osteria|pizzer|sushi|kebab/i, type: "restaurant", keyword: "ristorante" },
  ...
  { test: /panetter|forn|bakery/i, type: "bakery", keyword: "panetteria" },
...
function renderNearbyPlaces(list, target) {
  ...
  list.forEach(p => {
    html += `<div class="service-card glass" style="padding:1rem;border-radius:12px;">
      ...
      <span style="color:gold;">⭐ ${p.rating.toFixed(1)}</span> · ${p.total} recensioni · 📍 ${formatDistance(p.distanceM)}</div></div>`;
  });
  html += `</div>`;
  ...
  let color = "#FFC107";
  let trend = "😐 Hai un punteggio simile alla media locale.";
  if (diffR < -0.2 || diffT < -20) {
    color = "#FF5252";
    trend = "⚠️ Il tuo rating è inferiore alla media nella tua zona.";
  }
